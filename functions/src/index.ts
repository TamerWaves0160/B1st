...
function recsFrom(x: unknown): Rec[] {
  ...
  return x.map((r) => (isObject(r) ? r : {})).map((r) => ({
...
function narrativeFrom(dataset: Dataset, top: { type: string; count: number }[], totalDur: number) {
...
function mergeRec(incoming: Rec[] | undefined, seeds: Rec[]): Rec[] {
  ...
  if (Array.isArray(incoming)) {
    for (const r of incoming) {
      ...
      if (!key) continue;
      if (!seen.has(key)) { out.push({ title: r.title, rationale: r.rationale }); seen.add(key); }
    }
  }
  return out;
}
...
export const generateFbaBipDraft = onCall(
  ...
  async (request) => {
    ...
    const valid = {
      dataset: isDataset(dRaw),
      insights: isObject(iRaw),
      plan: isObject(pRaw),
    };

    if (!valid.dataset || !valid.insights || !valid.plan) {
      logger.warn("generateFbaBipDraft invalid payload", {
        uid,
    ...
    const draft = {
      ...
      narrative: {
        fbaSummary: narrativeBits.fbaSummary,
        bipPlan: narrativeBits.bipPlan,
    ...
    logger.info("generateFbaBipDraft VALIDATED", {
      uid,
    ...
    return {
      meta: { ...meta, serverReceivedAt: new Date().toISOString(), path: "validated" },
      draft,
    };
  }
);
